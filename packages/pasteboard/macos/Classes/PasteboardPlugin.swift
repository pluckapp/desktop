...
public class PasteboardPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    private func rawString(result: FlutterResult) {
        guard let str = NSPasteboard.general.readObjects(forClasses: [NSString.self], options: nil)?.first as? NSString else {
            result(nil)
            return
        }
        result(str)
